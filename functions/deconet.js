...
exports.getDecoOrders = functions
  ...
  .onRun(async () => {
    ...
    //     .then(({ status, data }) => {
    //       if (status > 200) {
    //         console.log(JSON.stringify({ message: 'Couldnt place deco order in Delta', status, data, o }));
    //       }
    //       return data;
    //     })
    //     .catch((err) => {
    ...
    const results = await Promise.allSettled(workOrders.map(sendToMSAS));
    console.log(results);
    ...
    //   .flat()
    //   .filter((o) => o);
    // console.log(JSON.stringify(deltaOrders));
    // const deltaResults = await Promise.allSettled(deltaOrders.map(sendToDelta));

    // console.log(JSON.stringify(deltaResults));
  });
